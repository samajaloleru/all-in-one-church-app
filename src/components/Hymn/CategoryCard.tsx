...
const HymnCategoryCard: React.FC<HymnCategoryCardProps> = ({ category, onClick }) => {
  return (
    <div 
      ...
      onClick={onClick}
    >
      <h3 className="text-xl font-bold text-zinc-800 mb-2">{category.title}</h3>
      <p className="text-rose-600">Hymns {category.range}</p>
      {category.description && (
        <p className="text-sm text-rose-500 mt-2">{category.description}</p>
      )}
    </div>
  );
};
